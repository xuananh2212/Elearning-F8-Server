...
function splitMediaToHLS(
  ...
  const ffmpegCommand = ffmpeg(inputMediaPath)
    ...
    .outputOptions([
      ...
      `${outputFolder}/${uniqueId}_%03d.ts`,
    ]);
      ...
      (err) => {
        if (err) {
          return res.status(500).json({
            error: "An error occurred during the HLS conversion process.",
          });
        }
